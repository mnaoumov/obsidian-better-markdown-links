...
- **`src/`** — plugin source:
  ...
  - `commands/` — twelve `CommandHandler` subclasses: convert links, convert links to Markdown, convert link paths to relative, and demote embeds, each in file, in folder, and in entire vault. The convert-to-Markdown and convert-paths-to-relative triads are palette-only (they do not override `shouldAddToFileMenu` / `shouldAddToFolderMenu`, whose base defaults are `false`), because a second and third nearly identically named row beside their style-agnostic sibling would cost more than they explain
