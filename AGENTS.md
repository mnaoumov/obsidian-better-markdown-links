...
- **`src/`** — plugin source:
  ...
  - `plugin-settings.ts` — `PluginSettings` model (angle brackets, leading dot/slash, `linkConversionMode`, include/exclude paths, link style, `shouldNormalizeFileLinks` for `file://` link normalization, `shouldAppendFileNameWhenDemotingEmbeds`, `shouldResolveLinksViaAliases`, `shouldCreateMissingNotes`, plus the two rename-handover bookkeeping keys)
  ...
  - `commands/` — six `CommandHandler` subclasses: convert links, and demote embeds, each in file, in folder, and in entire vault
