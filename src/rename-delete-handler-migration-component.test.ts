...
const DEFAULT_EXCLUDE_PATHS = [String.raw`/.+\.excalidraw\.md$/`, String.raw`/.+\.tldraw\.md$/`];
const SOURCE_PLUGIN_ID = 'better-markdown-links';
...
describe('RenameDeleteHandlerMigrationComponent', () => {
  ...
  // The path settings scoped this plugin's own handler, so they travel with the toggle — the vault-wide
  // Handler needs them to keep behaving the way this plugin did.
  ...
  // The paths are proposed, not moved — this plugin keeps its own copies, which still scope link conversion.
  it('should keep its own path settings after the migration is applied', async () => {
    ...
    expect(settings.excludePaths).toEqual(DEFAULT_EXCLUDE_PATHS);
  });
