...
// eslint-disable-next-line import-x/first, import-x/imports-first -- vi.mock must precede imports.
import { ConvertLinksInSelectionCommandHandler } from './convert-links-in-selection-command-handler.ts';
...
describe('ConvertLinksInSelectionCommandHandler', () => {
  ...
  it('should be the selection sibling of the file, folder and vault commands', () => {
    expect(handler.id).toBe('convert-links-in-current-selection');
    expect(handler.name).toBe('Convert links in current selection');
  ...
  it('should offer itself in the editor menu on exactly the terms it can execute on', () => {
    ...
    expect(handler.shouldAddToEditorMenu(createEditor(true), context)).toBe(true);
    expect(handler.shouldAddToEditorMenu(createEditor(false), context)).toBe(false);
  });
