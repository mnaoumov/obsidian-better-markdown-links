...
// eslint-disable-next-line import-x/first, import-x/imports-first -- vi.mock must precede imports.
import { ConvertLinkPathsToRelativeInSelectionCommandHandler } from './convert-link-paths-to-relative-in-selection-command-handler.ts';
...
describe('ConvertLinkPathsToRelativeInSelectionCommandHandler', () => {
  ...
  it('should be the selection sibling of the file, folder and vault commands', () => {
    expect(handler.id).toBe('convert-link-paths-to-relative-in-current-selection');
    expect(handler.name).toBe('Convert link paths to relative in current selection');
    expect(handler.icon).toBe('folder-tree');
  });
