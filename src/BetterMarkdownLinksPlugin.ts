...
import {
  generateMarkdownLinkForPlugin,
  type GenerateMarkdownLinkFn,
  type GenerateMarkdownLinkForPluginOptions
...
export default class BetterMarkdownLinksPlugin extends PluginBase<BetterMarkdownLinksPluginSettings> {
  ...
  protected override createDefaultPluginSettings(this: void): BetterMarkdownLinksPluginSettings {
  ...
  protected override onloadComplete(): MaybePromise<void> {
    ...
    this.registerEvent(this.app.vault.on("rename", (file, oldPath) => invokeAsyncSafely(this.handleRename(file, oldPath))));
