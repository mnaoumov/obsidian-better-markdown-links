 ...
 *
 * THREE shots. The plugin's whole argument is a before/after on the text of a
 * link, so the set is: what Obsidian writes, what this plugin writes instead,
 * and the commands that convert the links already in the vault.
 *
 * Every shot is SOURCE mode. The difference this plugin makes is invisible in
 * reading view by design — both spellings render as the same link — so a
 * reading-view frame would show nothing it touches.
 *
 * The staged note carries every messy form at once (percent-escaped spaces, a
 * relative path with no `./`, a wikilink, an embed), so the after-shot shows the
 * breadth of the conversion in one frame rather than needing four.
 */
